...
# Setup paths to find the 'database' sibling folder
...
def create_eba_schema(cur):
    """Creates hierarchy, metrics, and values tables if they don't exist."""
    cur.execute(
        """
...
def get_or_create_node(cur, name: str, parent_id: Optional[int]):
    """Returns the ID of a hierarchy level, creating it if it's missing."""
    cur.execute(
...
def sync_hierarchy(cur, data: Dict[str, Any], parent_id: Optional[int] = None):
    """Recursively processes the YAML into the database."""
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            cur.execute(
                """INSERT INTO metrics (name, dimensions, hierarchy_id) 
                   VALUES (%s, %s, %s) 
                   ON CONFLICT (name) DO UPDATE SET dimensions = EXCLUDED.dimensions;""",
                (key, json.dumps({"friendly_name": value}), parent_id),
            )
...
def run_setup():
    ...
    try:
        ...
        conn.commit()
        print(
            f"✅ Metadata synced successfully in '{target_db if target_db else 'default'}'."
        )
